 ...
 *
 * This can optionally use the SIMD-math-prims library, if the macro
 * APPROXIMATE_MATH is defined. That seems to speed up gamma distribution
 * sampling by about 20%, or random_unnormalized_log_categorical32 by
 * about 250%.
 *
...
// http://arxiv.org/pdf/1304.3800.pdf <-- should be perfect for n+alpha where
// alpha << 1 and n is integer (BUT: uses O(alpha) uniform samples! possibly
// still useful for common cases 1 < alpha < 10 though)
...
static inline double random_exp64(random_state *state, const double lambda) {
    return -log(random_uniform64(state)) / lambda;
}
...
static void random_log_dirichlet64(
        random_state *state, size_t d, const double *alpha, double *x)
{
    double x_sum = 0.0;
    for (size_t i=0; i<d; i++) {
        if (alpha[i] < 0.6) {
            x[i] = random_log_gamma_small64(state, alpha[i]);
            x_sum += exp(x[i]);
        } else {
            const double exp_x = random_gamma64(state, alpha[i]);
            x_sum += exp_x;
            x[i] = log(exp_x);
        }
    }
    const double scale = -log(x_sum);
    for (size_t i=0; i<d; i++)
        x[i] += scale;
}


static void random_log_dirichlet32(
        random_state *state, size_t d, const float *alpha, float *x)
{
    float x_sum = 0.0;
    for (size_t i=0; i<d; i++) {
        if (alpha[i] < 0.6f) {
            x[i] = random_log_gamma_small32(state, alpha[i]);
            x_sum += expf(x[i]);
        } else {
            const float exp_x = random_gamma32(state, alpha[i]);
            x_sum += exp_x;
            x[i] = logf(exp_x);
        }
    }
    const float scale = -logf(x_sum);
    for (size_t i=0; i<d; i++)
        x[i] += scale;
}
